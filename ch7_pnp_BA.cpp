...
int main()
{
     ...
     * 
     */
    myVO::PnP_g2o pnp_solver(K_intrinsic);
    for(int i=2; i<NUM_OF_IMG;i++)
    {
        ...
        //myVO::findPos3d2d_GN(pts_3D_current,key_matched[i],K_intrinsic,R,t);
        pnp_solver.run(pts_3D_current,key_matched[i],R,t);
        // cv::Mat r_cv,t_cv,R_cv;
        // cv::solvePnP(pts_3D_current,key_matched[i],K,cv::Mat(),r_cv,t_cv,false);
