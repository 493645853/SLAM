...
{
    cv::Mat cameraMatrix, newCameraMat, distCoeffs;
    mono::load_caliParameters("/home/lx6/MyWork/SLAM/learn_slam/Calibration/", cameraMatrix, distCoeffs, newCameraMat);

    cv::VideoCapture capture(0);
    capture.set(cv::CAP_PROP_FRAME_WIDTH, 640);
    capture.set(cv::CAP_PROP_FRAME_HEIGHT, 480);
    cv::Mat frame_now, frame_last, frame_dist;
    ...
    while (1)
    {
        // update img frames
        capture >> frame_dist;
        cv::undistort(frame_dist,frame_now,cameraMatrix,distCoeffs);
        ...
        // match descripters
        if (descptr_now.cols == descptr_last.cols)
        {
