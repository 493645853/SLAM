...
int main()
{
    ...
    if(!fin)
    {
        std::cout<<"No trajectory file can be found at "<<trajectory_file<<std::endl;
        return 1;
    }
    ...
    while(!fin.eof())
    {
        ...
        Eigen::Isometry3d T(Eigen::Quaterniond(qx,qy,qz,qw));
        T.pretranslate(Eigen::Vector3d(tx,ty,tz));
        poses.push_back(T);
    }
    std::cout<<"Total txt size : "<<poses.size()<<std::endl;
